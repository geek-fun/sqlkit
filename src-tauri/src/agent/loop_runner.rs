...
async fn run_agent_loop_inner(
    ...
    let chat_msgs = build_llm_messages(&history, system_prompt.as_deref());
        ...
        let mut body = formatter.build_request(model, system_prompt.as_deref(), &chat_msgs, raw_tools, true);
        // Apply optional temperature/max_tokens from settings
        if let Some(temp) = settings.get("temperature").and_then(|v| v.as_f64()) {
            if let Some(obj) = body.as_object_mut() {
                obj.insert("temperature".to_string(), json!(temp));
            }
        }
        if let Some(mt) = settings.get("maxTokens").and_then(|v| v.as_u64()) {
            if let Some(obj) = body.as_object_mut() {
                ...
                Some(conn_id) => match connections.get(conn_id) {
                    Some(cfg) => {
                        if let Some(resolved_id) = cfg.get("connectionId").and_then(|v| v.as_str()) {
                            // Resolve from the fallback connectionConfig map
                            if let Some(resolved) = fallback_connection_config.get(resolved_id) {
                                resolved.clone()
                            } else {
                                cfg.clone()
                            }
