    ...
    tools.iter().filter_map(|t| {
        ...
        Some(ChatCompletionTool {
            ...
            function: FunctionObject {
                ...
                description: f.get("description").and_then(|d| d.as_str()).map(|s| s.to_string()),
...
pub async fn run_agent_step(
    ...
    let result = match api_compat {
        "anthropic" => run_anthropic(&window, &request_id, &model, &messages, &tools, &resolved_base, &api_key).await,
        _ => run_openai(&window, &request_id, &model, &messages, &tools, &resolved_base, &api_key, http_proxy.as_deref(), proxy_mode.as_deref()).await,
    }?;
    serde_json::to_string(&result).map_err(|e| format!("Serialize failed: {}", sanitize_error(&e.to_string())))
}
...
async fn run_openai(
    ...
    let http_client = create_http_client(proxy_mode.unwrap_or("none"), http_proxy.map(|s| s.to_string()), None, None);
    let config = async_openai::config::OpenAIConfig::new().with_api_key(api_key).with_api_base(base_url);
    ...
    let mut stream = client.chat().create_stream(request).await.map_err(|e| format!("Stream error: {}", e))?;
    ...
    while let Some(item) = stream.next().await {
        ...
        let cr = formatter.parse_chunk(&cv).map_err(|e| format!("Parse error: {}", e))?;
        if let Some(ref c) = cr.content_delta {
            if !c.is_empty() {
                let _ = window.emit("agent-delta", json!({"requestId": request_id, "content": c}));
                full.push_str(c);
            }
        }
        for tc in &cr.tool_calls {
            ...
            let _ = window.emit("agent-tool-call", json!({"requestId": request_id, "toolCall": p}));
            calls.push(p);
        }
...
fn parse_sse_data(line: &str) -> Option<Value> {
    let line = line.trim();
    if line.is_empty() || !line.starts_with("data: ") { return None; }
...
async fn run_anthropic(
    ...
    let client = create_http_client("none", None, None, Some(std::time::Duration::from_secs(300)));
    let mut headers = HeaderMap::new();
    headers.insert(CONTENT_TYPE, HeaderValue::from_static("application/json"));
    headers.insert("x-api-key", HeaderValue::from_str(api_key).map_err(|e| format!("Bad key: {}", e))?);
    ...
    let msgs: Vec<Value> = messages.iter().filter_map(|m| {
    ...
    if !anthropic_tools.is_empty() { body["tools"] = json!(anthropic_tools); }

    let url = format!("{}/v1/messages", base_url.trim_end_matches('/').trim_end_matches("/v1"));
    let resp = client.post(&url).headers(headers).json(&body).send().await.map_err(|e| format!("Req failed: {}", e))?;
    ...
    while let Some(item) = stream.next().await {
        ...
        loop {
            ...
            if line.trim().is_empty() || line.starts_with("event: ") { continue; }
            let Some(data) = parse_sse_data(&line) else { continue };
            match data.get("type").and_then(|t| t.as_str()) {
                Some("content_block_start") => {
                    if data.get("content_block").and_then(|b| b.get("type")).and_then(|t| t.as_str()) == Some("tool_use") {
                        let id = data.get("content_block").and_then(|b| b.get("id")).and_then(|v| v.as_str()).unwrap_or("").to_string();
                        let nm = data.get("content_block").and_then(|b| b.get("name")).and_then(|v| v.as_str()).unwrap_or("").to_string();
                        tool_use = Some((id, nm, String::new()));
                    }
                }
                Some("content_block_delta") => {
                    if let Some(text) = data.get("delta").and_then(|d| d.get("text")).and_then(|v| v.as_str()) {
                        if !text.is_empty() {
                            let _ = window.emit("agent-delta", json!({"requestId": request_id, "content": text}));
                            full.push_str(text);
                        }
                    }
                    if data.get("delta").and_then(|d| d.get("type")).and_then(|t| t.as_str()) == Some("input_json_delta") {
                        let partial = data.get("delta").and_then(|d| d.get("partial_json")).and_then(|v| v.as_str()).unwrap_or("");
                        if let Some((_, _, ref mut acc)) = tool_use { acc.push_str(partial); }
                    }
                }
                ...
                Some("error") => {
                    let msg = data.get("error").and_then(|e| e.get("message")).and_then(|v| v.as_str()).unwrap_or("Unknown");
...
pub async fn validate_llm_config(
    ...
    match api_compat {
        "anthropic" => {
            let url = format!("{}/v1/messages", resolved_base.trim_end_matches('/').trim_end_matches("/v1"));
            let body = json!({"model": model, "max_tokens": 10, "messages": [{"role": "user", "content": "ping"}]});
            let mut headers = HeaderMap::new();
            headers.insert(CONTENT_TYPE, HeaderValue::from_static("application/json"));
            headers.insert("x-api-key", HeaderValue::from_str(&api_key).map_err(|e| sanitize_error(&e.to_string()))?);
            headers.insert("anthropic-version", HeaderValue::from_static("2023-06-01"));
            let resp = client.post(&url).headers(headers).json(&body).send().await.map_err(|e| format!("Req failed: {}", sanitize_error(&e.to_string())))?;
            Ok(resp.status().is_success())
        }
        _ => {
            let url = format!("{}/models", resolved_base.trim_end_matches('/'));
            let resp = client.get(&url).header(AUTHORIZATION, format!("Bearer {}", api_key)).send().await.map_err(|e| format!("Req failed: {}", sanitize_error(&e.to_string())))?;
            Ok(resp.status().is_success())
        }
    }
}
