...
pub async fn execute_migration<A1: DatabaseAdapter, A2: DatabaseAdapter>(
    ...
    emit_progress(app_handle, &create_progress("migration", "preparing", 0, None, 0));

    for (table_idx, table_plan) in request.table_plans.iter().enumerate() {
        emit_progress(app_handle, &TransferProgress {
            ...
            message: Some(format!("Migrating table {} of {}", table_idx + 1, request.table_plans.len())),
        });
    ...
    emit_progress(app_handle, &create_progress("migration", "finalizing", total_processed, None, start_time.elapsed().as_millis() as u64));
...
async fn migrate_table<A1: DatabaseAdapter, A2: DatabaseAdapter>(
    ...
    let source_columns: Vec<String> = table_plan.column_mappings.iter()
        .map(|m| m.source_column.clone())
        .collect();

    let target_columns: Vec<String> = table_plan.column_mappings.iter()
        .map(|m| m.target_column.clone())
        .collect();

    let _schema_prefix = request.target_schema.as_ref()
        .map(|s| format!("\"{}\".", s))
        .unwrap_or_default();

    let count_query = format!(
        "SELECT COUNT(*) AS count FROM {}\"{}\"",
        request.source_schema.as_ref().map(|s| format!("\"{}\".", s)).unwrap_or_default(),
        table_plan.source_table
    );

    let count_result = source_adapter.execute_query(&count_query).await.map_err(|e| e.to_string())?;
    ...
    let col_list = source_columns.iter()
        .map(|c| format!("\"{}\"", c))
        .collect::<Vec<_>>()
        .join(", ");
    ...
    while offset < total_rows {
        ...
        let result = source_adapter.execute_query(&query).await.map_err(|e| e.to_string())?;
        ...
        emit_progress(app_handle, &TransferProgress {
            ...
            percent: if total_rows > 0 { (processed_rows as f32 / total_rows as f32) * 100.0 } else { 0.0 },
            elapsed_ms: start_time.elapsed().as_millis() as u64,
            estimated_remaining_ms: None,
            message: None,
        });
...
async fn insert_batch_to_target<A: DatabaseAdapter>(
    ...
    let schema_prefix = request.target_schema.as_ref()
        .map(|s| format!("\"{}\".", s))
        .unwrap_or_default();
    ...
    let values_list: Vec<String> = rows.iter()
        .map(|row| {
            let vals: Vec<String> = source_columns.iter()
                .map(|col| {
                    match row.get(col) {
    ...
    let result = target_adapter.execute_query(&sql).await.map_err(|e| e.to_string())?;
...
pub async fn preview_migration<A: DatabaseAdapter>(
    ...
    for table_plan in &request.table_plans {
        ...
        let count_result = source_adapter.execute_query(&count_query).await.map_err(|e| e.to_string())?;
        let row_count = count_result.rows.first()
            .and_then(|row| row.get("count"))
            .and_then(|v| match v {
                QueryValue::Int(n) => Some(*n as u64),
                _ => None,
        ...
        let conversions = table_plan.column_mappings.iter()
