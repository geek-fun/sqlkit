...
case "$TARGET" in
  universal-apple-darwin)
    ...
    jq -n \
      --arg version "$VERSION" \
      --arg sig "$SIG" \
      --arg url "https://github.com/geek-fun/sqlkit/releases/download/v${VERSION}/${TAR_NAME}" \
  ...
  x86_64-pc-windows-msvc)
    ...
    mv "${EXE_FILES[0]}" "artifacts/SqlKit_${VERSION}_x64-setup.exe"
    jq -n \
      --arg version "$VERSION" \
      --arg sig "$SIG" \
      --arg url "https://github.com/geek-fun/sqlkit/releases/download/v${VERSION}/SqlKit_${VERSION}_x64-setup.exe" \
  ...
  x86_64-unknown-linux-gnu)
    ...
    mv "${BUNDLE_DIR}"/deb/*.deb "artifacts/SqlKit_${VERSION}_amd64.deb"
    mv "${APPIMAGE_FILES[0]}" "artifacts/SqlKit_${VERSION}_amd64.AppImage"
    jq -n \
      --arg version "$VERSION" \
      --arg sig "$SIG" \
      --arg url "https://github.com/geek-fun/sqlkit/releases/download/v${VERSION}/SqlKit_${VERSION}_amd64.AppImage" \
