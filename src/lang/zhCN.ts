  ...
  pages: {
    ...
    settings: {
      ...
      mcp: {
        ...
        restart: '重启桥接',
        restartSuccess: 'MCP 桥接重启成功',
        restartFailed: '重启失败',
        restartFailedDetail: 'MCP 桥接重启失败',
        restartTimeout: 'MCP 桥接 60 秒内未能启动',
        shuttingDown: '正在关闭...',
        starting: '正在启动...',
        autoStart: '自动启动',
        autoStartDesc: '应用启动时自动启动 MCP 桥接',
        permissionMode: '权限模式',
        permissionModeDesc: '控制 MCP 客户端通过桥接可执行的操作',
        modeReadOnly: '只读',
        modeReadOnlyDesc: '仅允许查询，不允许修改',
        modeDataReadWrite: '数据读写',
        modeDataReadWriteDesc: '允许 CRUD，不允许 schema 变更',
        modeFullAccess: '完全访问',
        modeFullAccessDesc: '允许所有操作',
        confirmDestructive: '确认破坏性操作',
        confirmDestructiveDesc: '执行破坏性操作（DROP、TRUNCATE、DELETE 等）前需要明确确认',
        allowlist: '连接白名单',
        allowlistDesc: '选择 MCP 客户端可访问的连接。留空表示允许所有连接。',
        allowlistEmpty: '未选择任何连接 — 所有连接都可访问',
        connectionOverrides: '连接覆盖',
        overrideReadOnly: '只读',
        overrideReadOnlyDesc: '无论全局模式如何，强制该连接为只读访问',
        saveFailed: '保存 MCP 设置失败',
        connectionAccess: '连接访问控制',
        connectionAccessDesc:
          '启用后仅允许列表中的连接。可为每个连接设置操作权限（读/写/删除）。',
        connectionName: '连接',
        connectionType: '类型',
