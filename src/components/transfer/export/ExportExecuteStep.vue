...
async function startExport() {
  if (!transferStore.exportRequest.outputPath || !transferStore.exportRequest.connectionId) {
    return
  }
  ...
  try {
    const res = await executeExport(transferStore.exportRequest as any)
    result.value = res
...
function handleRunInBackground() {
  ...
  transferStore.detachActiveTask('export')
}
...
function handleAgain() {
  result.value = null
  progress.value = null
  transferStore.resetExport()
}

onMounted(async () => {
  unlistenProgress = await listen<TransferProgress>('transfer-progress', (event) => {
    progress.value = event.payload
    if (transferStore.activeExportTaskId) {
      transferStore.syncProgressToTask(transferStore.activeExportTaskId, event.payload)
    }
  })
...
<template>
  <div class="space-y-3">
    ...
    <ResultPanel
      ...
      :result="result"
      :output-path="transferStore.exportRequest.outputPath"
      @again="handleAgain"
    />
  </div>
</template>
