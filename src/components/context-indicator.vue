...
import type { ContextUsage } from '@/datasources/agentApi'
...
const percent = computed(() => {
  if (!usage.value || usage.value.capacity === 0)
    return 0
...
const severityClass = computed(() => {
  ...
  if (p >= 80)
    return 'context-indicator--critical'
  if (p >= 60)
    return 'context-indicator--warning'
...
const statusText = computed(() => {
  if (!usage.value)
    return t('dataStudio.contextIndicator.statusIdle') || 'Idle'
  if (autoCompactOff.value) {
    return usage.value.should_compact
      ? (t('dataStudio.contextIndicator.statusOverThresholdOff') || 'Over threshold, compact off')
      : (t('dataStudio.contextIndicator.statusAutoCompactOff') || 'Auto-compact off')
  }
  if (usage.value.should_compact)
    return t('dataStudio.contextIndicator.statusWillAutoCompact') || 'Will auto-compact'
  return t('dataStudio.contextIndicator.statusHealthy') || 'Healthy'
})

const tooltip = computed(() => {
  if (!usage.value)
    return t('dataStudio.contextIndicator.tooltipIdle') || 'Context usage'
  const base = `Context ${percentText.value} · ${formatTokens(usage.value.used_tokens)} / ${formatTokens(usage.value.capacity)}`
  return autoCompactOff.value
    ? `${base} · ${t('dataStudio.contextIndicator.tooltipAutoCompactOff') || 'Auto-compact off'}`
    : base
})
...
async function refresh() {
  if (!props.sessionId || !props.settings)
    return
  try {
...
async function onCompact() {
  if (!props.sessionId || !props.settings || compacting.value)
    return
  ...
  try {
    ...
    usage.value = next
    emit('compacted', next)
    toast.success(t('dataStudio.contextIndicator.compactSuccess') || 'Context compacted')
  }
  catch (err) {
    const msg = typeof err === 'string' ? err : String(err)
    toast.error((t('dataStudio.contextIndicator.compactFailed') || 'Compact failed: {error}').replace('{error}', msg))
  }
  finally {
  ...
  <div class="context-indicator" :class="severityClass">
    ...
    >
      ...
      <span
        ...
        :title="t('dataStudio.contextIndicator.offBadgeTitle') || 'Auto-compact is off'"
      >
        OFF
      </span>
    ...
    <div v-if="open" class="context-indicator__popover" @click.stop>
      <header class="context-indicator__popover-header">
        <span class="context-indicator__popover-title">
          {{ t('dataStudio.contextIndicator.popoverTitle') || 'Context Usage' }}
        </span>
      ...
      <div v-if="autoCompactOff && usage?.should_compact" class="context-indicator__warning">
        ...
        <span>{{ t('dataStudio.contextIndicator.warningText') || 'Context is over threshold but auto-compact is off' }}</span>
      </div>

      <dl class="context-indicator__grid">
        <div>
          <dt>{{ t('dataStudio.contextIndicator.labelUsed') || 'Used' }}</dt>
          <dd>{{ formatTokens(usage?.used_tokens ?? 0) }}</dd>
        </div>
        <div>
          <dt>{{ t('dataStudio.contextIndicator.labelCapacity') || 'Capacity' }}</dt>
          <dd>{{ formatTokens(usage?.capacity ?? 0) }}</dd>
        </div>
        <div>
          <dt>{{ t('dataStudio.contextIndicator.labelWindow') || 'Window' }}</dt>
          <dd>{{ formatTokens(usage?.context_window ?? 0) }}</dd>
        </div>
        <div>
          <dt>{{ t('dataStudio.contextIndicator.labelReservedOut') || 'Output Reserve' }}</dt>
          <dd>{{ formatTokens(usage?.output_reserve ?? 0) }}</dd>
        </div>
        <div>
          <dt>{{ t('dataStudio.contextIndicator.labelAutoCompactAt') || 'Auto-Compact At' }}</dt>
          <dd>{{ formatTokens(usage?.trigger_at ?? 0) }}</dd>
        </div>
        <div>
          <dt>{{ t('dataStudio.contextIndicator.labelStatus') || 'Status' }}</dt>
          <dd>{{ statusText }}</dd>
        </div>
      </dl>
      ...
      <footer class="context-indicator__popover-footer">
        ...
        >
          ...
          <span>
            {{
              compacting
                ? (t('dataStudio.contextIndicator.compacting') || 'Compacting...')
                : (t('dataStudio.contextIndicator.compactNow') || 'Compact Now')
            }}
          </span>
        </button>
      </footer>
